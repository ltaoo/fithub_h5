...
import { ArrowDown, ArrowUp, Trash } from "lucide-solid";
...
import { WorkoutActionMultipleSelectView } from "@/components/workout-action-multiple-select";
  ...
  return (
    <div class="w-full">
      {(() => {
        ...
        if (state().input?.type === "workout_action_select") {
          return <WorkoutActionMultipleSelectView store={store.input} />;
        }
        if (state().input?.type === "workout_action_select2") {
          return <WorkoutActionSelectView store={store.input} />;
        }
