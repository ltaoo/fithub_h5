...
export const SetDropdownMenu = (props: { store: DropdownMenuCore } & JSX.HTMLAttributes<HTMLElement>) => {
  const { store } = props;
    ...
    <DropdownMenuPrimitive.Root store={store}>
      ...
      <DropdownMenuPrimitive.Portal store={store.menu}>
        <DropdownMenuPrimitive.Content
          class={cn(
            "z-50 min-w-[88px] overflow-hidden rounded-md border-2 border-slate-100 bg-white p-1 text-slate-700 shadow-md dark:border-slate-800 dark:bg-slate-800 dark:text-slate-400",
            props.class
          )}
          store={store}
        >
