      ...
      <div class="flex flex-wrap items-center gap-2">
        <For each={state().value}>
          {(muscle) => (
            <div class="flex items-center gap-2 bg-slate-100 rounded-md p-1">
              <span class="text-sm whitespace-nowrap text-slate-400">{muscle.zh_name || muscle.name}</span>
              <button
                class="text-slate-400"
                onClick={() => {
                  vm.methods.remove(muscle);
                }}
              >
                <X class="w-4 h-4" />
              </button>
        ...
        <button
          ...
          onClick={(e) => {
            ...
            console.log("[MUSCLE-SELECT] onAnimationEnd", left, top, width, height);
