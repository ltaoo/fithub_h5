...
export function RepsInputModel(props: {
  defaultValue: string;
  suffix: SetValueUnit;
  ...
  const ui = {
    ...
    $select: new SelectCore({
      defaultValue: props.suffix ?? getSetValueUnit("次"),
      options: RepsSetValueOptions,
      onChange(v) {
        if (v === getSetValueUnit("ToFail")) {
          ui.$keyboard.disable();
        } else {
          ui.$keyboard.enable();
        }
  ...
  return {
    ...
    setValue(v: { num: string; unit: SetValueUnit }) {
      ...
      // console.log("before disabled", v.unit === getSetValueUnit("ToFail"), v.unit);
      if (v.unit === getSetValueUnit("ToFail")) {
        ui.$keyboard.disable();
      } else {
        ui.$keyboard.enable();
      }
